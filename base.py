    ...
    for file in os.listdir("base"):
        if file.endswith((".csv", ".xls", ".xlsx", ".txt")):  # Added .xlsx to the check
            if file.endswith(".csv"):
                try:
                    with open(os.path.join("base", file), 'r', encoding='utf-8', errors='ignore') as f:
                        reader = csv.reader(f)
                        for row in reader:
                ...
                try:
                    wb = xlrd.open_workbook(os.path.join("base", file))
                ...
                try:
                    wb = load_workbook(os.path.join("base", file))
...
search_query = input(Fore.RED + "                                                                                                Введите данные : " + Style.RESET_ALL)
